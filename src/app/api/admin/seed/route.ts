...
type SeedTeam = { prefix: string; paginas: [number, number]; count: number; nome: string };
...
const EXTRA_FWC = [
  { paginas: [106, 107] as [number, number], start: 9, end: 13 },
  { paginas: [108, 109] as [number, number], start: 14, end: 19 }
];
...
export async function POST() {
  try {
    ...
    const ops: { codigo: string; pagina: number; nomeSelecao: string }[] = [];
    ...
    for (const e of EXTRA_FWC) {
      ...
      const a = nums.slice(0, firstPageCount);
      const b = nums.slice(firstPageCount);
      a.forEach((n) => ops.push({ codigo: `FWC${n}`, pagina: p1, nomeSelecao: "FWC" }));
      b.forEach((n) => ops.push({ codigo: `FWC${n}`, pagina: p2, nomeSelecao: "FWC" }));
    }
    ...
    return jsonError("Erro ao seedar", 500);
  }
}
